...
# # Train new Magnetization model with RFR + bagging
#
...
from src.util import load_experimental
...
FEATURES = ['Nd_s', 'Ce_s', 'La_s', 'Pr_s', 'Y_s', 'Tb_s', 'Dy_s', 'Fe_s', 'Co_s', 'Ni_s', 'B_s', 'C_s', 'temp_s']
DATA_PATH = (Path(__file__).parents[1] / "data/tmc").absolute()
np.random.seed(SEED)
random.seed(SEED)
...
class CustomBaggingRegressor(BaseEstimator, RegressorMixin):
    """Custom BaggingRegressor that always predicts with std."""
...
def split_and_scale_data(X, y, df, seed):
    ...
    df_processed[X_train.columns] = scaler.transform(df_processed[X_train.columns])
...
def write_model_summary_to_file(regressor, model_summary_path, metrics_by_seed_df, metrics_summary_df):
    bagging_step = regressor.model_ if hasattr(regressor, "model_") else regressor
    base_step = bagging_step.estimator

    lines = [
        "Model Summary",
        "=" * 80,
        "BaggingRegressor Summary",
        "*" * 80,
        f"Representative seed for plots/model export: {SEED}",
        f"Evaluation seeds for reported metrics: {EVAL_SEEDS}",
        f"Base Estimator: {base_step}",
        f"Number of Bagging Estimators: {bagging_step.n_estimators}",
        f"Random State: {bagging_step.random_state}",
        f"n_jobs: {bagging_step.n_jobs}",
    ]

    if hasattr(base_step, "n_estimators"):
        lines.append(f"Trees per RandomForest: {base_step.n_estimators}")
    if hasattr(base_step, "max_depth"):
        lines.append(f"Max Depth: {base_step.max_depth}")
    if hasattr(base_step, "max_features"):
        lines.append(f"Max Features: {base_step.max_features}")
...
def main():
    ...
    print(f"[*] evaluating RFR + bagging across {len(EVAL_SEEDS)} random seeds ...")
    ...
    df_processed = representative_artifacts["df_processed"]
    ...
    print(f"Base Estimator: {regressor.estimator}")
    print(f"Bagging Estimators: {regressor.model_.n_estimators}")
    print(f"Stored Estimators: {len(regressor.model_.estimators_)}")
    print()
    ...
    plot_confidence_curve(
        ...
        title=f"Confidence curve of RFR + bagging model on test-set ({SYMBOL})",
        seed=SEED,
